...
**Docker:** **`docker/gpu`** builds **`prismalama-gpu`** (Go + GGML HIP/Vulkan, Ubuntu/ROCm base). **`docker/arch`** builds **`prismalama-arch`** from the **same `PKGBUILD`** (full **`makepkg`** in Docker, or **`Dockerfile.prebuilt`** after **`cp prismalama-ollama-*.pkg.tar.zst docker/arch/prismalama.pkg.tar.zst`**) so the container matches a native Arch install. Neither image ships Python **`transformers`** by default; extend the image or use the host if you need AirLLM. If you need AirLLM in a container, add **`pip install transformers safetensors`**, **`python-pytorch-rocm`** (or CUDA), and run **`ollama serve`** there.
