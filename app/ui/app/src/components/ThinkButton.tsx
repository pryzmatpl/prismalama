...
export const ThinkButton = forwardRef<HTMLButtonElement, ThinkButtonProps>(function ThinkButton(
  { mode, isVisible, isActive, currentLevel, onToggle, onLevelChange, onDropdownToggle },
  ref,
) {
  ...
  useEffect(() => {
    if (ref && typeof ref === "object" && ref.current && mode === "thinkingLevel") {
  ...
  // thinkingLevel mode
  const displayLabel = currentLevel ? THINKING_LEVEL_LABELS[currentLevel] : "";
  return (
          ...
          {Object.entries(THINKING_LEVELS).map(([, level]) => (
            <button
              ...
              className={`w-full text-left px-3 py-2 cursor-pointer hover:bg-neutral-100 dark:hover:bg-neutral-700 transition-colors text-neutral-700 dark:text-neutral-300 ${
                currentLevel === level ? "bg-neutral-100 dark:bg-neutral-700/60" : ""
              }`}
              onClick={() => {
                onLevelChange?.(level);
                setIsDropdownOpen(false);
