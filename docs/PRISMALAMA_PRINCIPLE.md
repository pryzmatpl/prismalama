...
The **product key** is not “pick GGUF *or* AirLLM forever.” It is:
...
2. **Evolve the GGUF path** so weights can be **streamed and time-multiplexed** in the same *sense* as AirLLM: load a **layer (or block)**, run **HIP/Vulkan GGML** ops, **evict / prefetch** from NVMe, repeat — without requiring PyTorch for models that are already GGUF.
