...
<Note>
  Ollama must be installed and running before the installer runs. When running inside WSL2 or a
  container, ensure Ollama is reachable from the sandbox (e.g. `OLLAMA_HOST=0.0.0.0`).
</Note>
| Platform | Runtime | Status |
|----------|---------|--------|
| Linux (Ubuntu 22.04+) | Docker | Primary |
