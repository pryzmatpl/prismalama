...
<Note>
  Some RDNA2 / Radeon RX 6000 systems, including RX 6800-class cards, may not expose ROCm v7 on
  current Windows AMD drivers. Vulkan is enabled by default and is the recommended fallback for
  those systems. If a mixed iGPU/dGPU system selects an unstable Vulkan iGPU, set
  `GGML_VK_VISIBLE_DEVICES` to the discrete GPU index.
</Note>
